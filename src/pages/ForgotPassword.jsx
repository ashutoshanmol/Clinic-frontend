...
const ForgotPassword = () => {
  ...
  const handleRequestOtp = async (e) => {
    ...
    try {
      const res = await axios.post('/api/public/forgot-password', { email });
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setMessage(''); setError(''); setLoading(true);
    try {
      const res = await axios.post('/api/public/verify-otp', { email, otp });
      setMessage(res.data.message);
      setStep(3);
    } catch (err) {
  ...
  const handleResetPassword = async (e) => {
    ...
    setMessage(''); setError(''); setLoading(true);
    try {
      const res = await axios.post('/api/public/reset-password', {
        email,
        otp,
        newPassword
      });
